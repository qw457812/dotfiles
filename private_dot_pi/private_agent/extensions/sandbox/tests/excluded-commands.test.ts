...
import { matchExcludedCommand } from "../excluded-commands.ts";
...
async function match(command: string, extraPatterns = patterns, workdir = cwd) {
  return matchExcludedCommand(command, workdir, extraPatterns);
}

test("excluded commands matcher", async () => {
  ...
  assert.equal(
    await match("/tmp/skills/web-browser/scripts/nav.js https://example.org --new"),
    null,
  );
