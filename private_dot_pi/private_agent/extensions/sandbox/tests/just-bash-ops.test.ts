...
import { __testing, createJustBashOps } from "../just-bash-ops.ts";

const thisFilePath = fileURLToPath(import.meta.url);
const thisDirPath = dirname(dirname(thisFilePath));

test("just-bash operations", async () => {
  const isTermux =
  ...
  async function runWithOps(
    selectedOps: ReturnType<typeof createJustBashOps>,
    command: string,
    timeout = 5,
  ) {
    let output = "";
  ...
  async function runErrorWithOps(
    selectedOps: ReturnType<typeof createJustBashOps>,
    command: string,
  ) {
    try {
  ...
  async function runError(command: string) {
    return runErrorWithOps(ops, command);
  }

  async function runBinary(command: string) {
    const chunks: Buffer[] = [];
  ...
  try {
    ...
    assert.deepEqual(
      await runWithOps(
        explicitDeviceOps,
        "printf discarded >/dev/null; printf device >/dev/stdout",
      ),
    ...
    assert.deepEqual(
      await run(`printf tmp > ${defaultTmpWritePath} && cat ${defaultTmpWritePath}`),
      {
        exitCode: 0,
        output: "tmp",
      },
    ...
    if (existsSync("/tmp")) {
      ...
      assert.match(
        String(literalTmpWrite.error),
        /ENOENT: no such file or directory, open '\/tmp\//,
      );
    ...
    assert.match(
      String(readonlyMissingParentWrite.error),
    ...
    await assert.doesNotReject(() =>
      __testing.assertReadonlyEntryExists(danglingSymlinkPath, "rm"),
    );
    await assert.doesNotReject(() =>
      __testing.assertReadonlyEntryExists(danglingSymlinkPath, "cp"),
    );
    ...
    try {
      ...
      const deniedPathExists = await runWithOps(
        deniedReadOps,
        `test -e ${hostSecretPath}; echo $?`,
      );
    ...
    writeFileSync(
      binaryProbe,
      "process.stdout.write(Buffer.from([0x41, 0xFF, 0x80, 0x00, 0x42]));",
    );
