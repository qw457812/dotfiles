...
if [ "$__IS_CLAUDECODE_NVIM" = "1" ] || [ -n "$TERMUX_VERSION" ]; then
  ...
  *) vim_mode_display=$([ -n "$vim_mode" ] && echo "${vim_mode}") ;;
  ...
  if echo "$syn_quota" | jq -e . >/dev/null 2>&1; then
    syn_5h_used=$(format_num "$(echo "$syn_quota" | jq -r '.five_hour.used // 0')")
    syn_5h_limit=$(echo "$syn_quota" | jq -r '.five_hour.limit // 0')
    syn_5h_regen_ms=$(echo "$syn_quota" | jq -r '.five_hour.regen_remaining_ms // 0')
    syn_5h_display="${COLOR_MAGENTA}${syn_5h_used}${COLOR_MAUVE}/${syn_5h_limit}/$(format_ms "$syn_5h_regen_ms")${COLOR_RESET}"
    ...
    syn_weekly_used_pct=$(echo "$syn_quota" | jq -r '.weekly.used_pct // 0')
    syn_weekly_regen_ms=$(echo "$syn_quota" | jq -r '.weekly.regen_remaining_ms // 0')
    syn_weekly_display="${COLOR_MAGENTA}$(format_num "$syn_weekly_used_pct")%${COLOR_MAUVE}/$(format_ms "$syn_weekly_regen_ms")${COLOR_RESET}"

    syn_quota_display=$(printf '%s\n' "$syn_5h_display" "$syn_tc_display" "$syn_weekly_display" | xargs)
  fi
